<template>
  <div class="form-common01" >
      ...
      <div class="form-group">
        ...
        <div class="box-form01 box-form-sort" :style="scenario.tags && scenario.tags.length >= 0? {display: 'block'}:{display: 'none'}" >
          ...
          <div class="list-checkbox-tag" v-if="refresh_tag">
            <input-tag :data="scenario.tags" @input="addListTag"></input-tag>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="form-bottom">
      <div class="row-form-btn flex">
        <button type="submit" class="btn btn-submit btn-block" @click="scenarioEdit()">保存</button>
         <button
            type="submit"
            class="btn btn-draft btn-block"
            @click="scenarioEdit('draft')"
          >下書き保存</button>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async scenarioEdit(status) {
      ...
      this.$store.dispatch('scenario/scenarioEdit', this.scenario).then((res) => {
        if (this.type === 'template') {
          window.location.href = process.env.MIX_ROOT_PATH + '/template/scenarios?is_updated=true';
        } else {
          window.location.href = process.env.MIX_ROOT_PATH + '/scenarios?is_updated=true';
        }
      }).catch(() => {
      });
    },
